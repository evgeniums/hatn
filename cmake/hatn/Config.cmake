...
IF(${BUILD_TYPE} STREQUAL "DEBUG")
    ...
    SET(LIB_POSTFIX d)
    # ADD_DEFINITIONS(-DHATN_BUILD_DEBUG)
...
OPTION(GENERATE_VERSION "Generate version information for the project" OFF)
OPTION(ENABLE_TRANSLATIONS "Enable translations (gettext is required)" OFF)
OPTION(STRIP_SYMBOLS "Strip symbols from release dynamic or exe binaries" ON)
...
IF (NOT CMAKE_CXX_STANDARD)
    ...
    IF ($ENV{CMAKE_CXX_STANDARD})
        ...
        MESSAGE(STATUS "Using CMAKE_CXX_STANDARD=${CMAKE_CXX_STANDARD} as requested in environment")
    ELSE()
...
IF ($ENV{HATN_TEST_VALGRIND})
    # ADD_DEFINITIONS(-DBUILD_VALGRIND)
...
IF (APPLE AND IOS_SDK_VERSION_X10)
    MESSAGE(STATUS "IOS_SDK_VERSION_X10 ${IOS_SDK_VERSION_X10}")
    # ADD_DEFINITIONS(-DIOS_SDK_VERSION_X10=${IOS_SDK_VERSION_X10})
...
IF(BUILD_ANDROID)
    ...
    SET(HATN_COMPILE_DEFINITIONS ${HATN_COMPILE_DEFINITIONS} -DANDROID)
ENDIF()
...
IF (MSVC AND BUILD_DEBUG)
    # SET(HATN_CXX_FLAGS_DEBUG "${HATN_CXX_FLAGS_DEBUG} /bigobj")
...
IF (NOT PYTHON_EXE)
    IF (NOT "$ENV{PYTHON_EXE}" STREQUAL "")
    	MESSAGE(STATUS "PYTHON_EXE is set in environment: $ENV{PYTHON_EXE}")
    	SET (PYTHON_EXE $ENV{PYTHON_EXE})
    ENDIF()
ELSE()
    MESSAGE (STATUS "Python is set from command line: " ${PYTHON_EXE})    
ENDIF()
...
IF(WIN32)
    # ADD_DEFINITIONS(-DWIN32 -D_WIN32_WINNT=0x0600 -Dstrcasecmp=stricmp)
    SET(HATN_COMPILE_DEFINITIONS ${HATN_COMPILE_DEFINITIONS} -DWIN32 -D_WIN32_WINNT=0x0600 -Dstrcasecmp=stricmp)
    IF(MSVC)
        # ADD_DEFINITIONS(-D_CRT_SECURE_NO_WARNINGS -D_SCL_SECURE_NO_WARNINGS -wd4251 -wd4275 -wd4996 -nologo)
        ...
        # SET(HATN_CXX_FLAGS "${HATN_CXX_FLAGS} -W3 -EHsc /wd4250 /wd4005 /wd4661 /Zc:__cplusplus /Zc:ternary")
    ENDIF(MSVC)
ENDIF(WIN32)

IF (NOT STATIC_BUILD)
    IF (NOT WIN32)
        # SET(HATN_CXX_FLAGS "${HATN_CXX_FLAGS} -fPIC")
...
ENDIF()
# SET(HATN_CXX_FLAGS_DEBUG "${HATN_CXX_FLAGS_DEBUG} -DBUILD_DEBUG")
# SET(HATN_CXX_FLAGS_RELEASE "${HATN_CXX_FLAGS_RELEASE}")

IF ((${CMAKE_CXX_COMPILER_ID} MATCHES "Clang"))
    # SET(HATN_CXX_FLAGS "${HATN_CXX_FLAGS} -Wno-unused-function -Qunused-arguments")
...
IF(NO_DYNAMIC_HATN_PLUGINS)
    # ADD_DEFINITIONS(-DNO_DYNAMIC_HATN_PLUGINS)
...
IF (TREAT_WARNING_AS_ERROR)
    ...
    IF(MSVC)
        # SET(HATN_CXX_FLAGS "${HATN_CXX_FLAGS} /WX")
        SET(HATN_COMPILE_OPTIONS ${HATN_COMPILE_OPTIONS} /WX)
    ELSE(MSVC)
        # SET(HATN_CXX_FLAGS "${HATN_CXX_FLAGS} -Werror")
        SET(HATN_COMPILE_OPTIONS ${HATN_COMPILE_OPTIONS} -Werror)
    ENDIF(MSVC)
ENDIF (TREAT_WARNING_AS_ERROR)

# SET (HATN_CXX_FLAGS_RELEASE "${CMAKE_CXX_FLAGS_RELEASE} ${HATN_CXX_FLAGS_RELEASE}")
# SET (HATN_CXX_FLAGS_DEBUG "${CMAKE_CXX_FLAGS_DEBUG} ${HATN_CXX_FLAGS_DEBUG}")
# SET (HATN_CXX_FLAGS "${CMAKE_CXX_FLAGS} ${HATN_CXX_FLAGS}")
...
# SET (CMAKE_CXX_FLAGS_DEBUG "${HATN_CXX_FLAGS_DEBUG}")
# SET (CMAKE_CXX_FLAGS_RELEASE "${HATN_CXX_FLAGS_RELEASE}")
# SET (CMAKE_CXX_FLAGS ${HATN_CXX_FLAGS})
