...
FUNCTION(LINK_HATN_PLUGIN TARGET_NAME PLUGIN_NAME export_mode)
    ...
    IF (NOT ${pluginname} STREQUAL ${TARGET_NAME})
        IF (BUILD_STATIC)
            ...
            TARGET_LINK_LIBRARIES(${TARGET_NAME} ${export_mode} ${pluginname})
        ELSE(BUILD_STATIC)
...
FUNCTION(LINK_HATN_PLUGINS TARGET_NAME MODULE export_mode)
    ...
    FOREACH(plugin_name ${BUILD_PLUGINS})
        IF ("${plugin_name}" STREQUAL "all")
            IF (EXISTS ${MODULE_PLUGINS_PATH})
                SUBDIRLIST(plugins_in_dir ${MODULE_PLUGINS_PATH})
                FOREACH(plugin ${plugins_in_dir})
                    LINK_HATN_PLUGIN(${TARGET_NAME} ${plugin} ${export_mode})
                    GET_PROPERTY(pl GLOBAL PROPERTY PLUGIN_LIST)
                    SET_PROPERTY(GLOBAL PROPERTY PLUGIN_LIST ${pl} "\"${plugin_name}\"")
                ENDFOREACH()
            ENDIF()
            BREAK()
        ELSE ()
            ...
            IF (EXISTS "${MODULE_PLUGINS_PATH}/${plugin_name}")
                ...
                LINK_HATN_PLUGIN(${TARGET_NAME} ${plugin_name} ${export_mode})
                GET_PROPERTY(pl GLOBAL PROPERTY PLUGIN_LIST)
                SET (pn "\"${plugin_name}\"")
                IF (NOT "${pn}" IN_LIST pl)
                    SET_PROPERTY(GLOBAL PROPERTY PLUGIN_LIST ${pl} ${pn})
                ENDIF()
            ENDIF()
        ENDIF()
    ENDFOREACH()
    ...
    IF(NumArgs GREATER 0)
        FOREACH(ARG ${Args})
            ...
            IF (
                ...
                LINK_HATN_PLUGINS(${target} ${ARG} ${export_mode})
            ELSE()
...
FUNCTION (READ_HATN_MODULE_TEST_DEPS module_name)
    STRING (TOUPPER ${module_name} UPPER_MODULE_NAME)
    SET(TEST_DEPS_CMAKE ${HATN_${UPPER_MODULE_NAME}_SRC}/test/depends.cmake)
    IF (EXISTS ${TEST_DEPS_CMAKE})
        INCLUDE(${TEST_DEPS_CMAKE})
    ENDIF()
ENDFUNCTION(READ_HATN_MODULE_TEST_DEPS)
...
FUNCTION(COPY_HATN_PLUGIN MODULE_NAME PLUGIN_NAME)

    MESSAGE(STATUS "Copying plugin \"${PLUGIN_NAME}\" for module \"${MODULE_NAME}\"")
