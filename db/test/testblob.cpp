...
HDU_UNIT(base_fields,
         ...
         HDU_FIELD(blob_field,TYPE_BYTES,4)
         )
...
HATN_DB_MODEL_WITH_CFG(modelNoP1,no_p,ModelConfig("no_p1").setBlobEnabled(true),df2Idx(),df3Idx())
HATN_DB_MODEL_WITH_CFG(modelNoP2,no_p,ModelConfig("no_p2").setBlobEnabled(true),df2Idx(),df3Idx())
HATN_DB_MODEL_WITH_CFG(modelNoP3,no_p,ModelConfig("no_p3").setBlobEnabled(true),df2Idx(),df3Idx())
HATN_DB_MODEL_WITH_CFG(modelNoP4,no_p,ModelConfig("no_p4").setBlobEnabled(true),df2Idx(),df3Idx())

HATN_DB_OID_PARTITION_MODEL_WITH_CFG(modelImplicit1,implicit_p,ModelConfig("implicit_p1").setBlobEnabled(true),df2Idx(),df3Idx())
HATN_DB_OID_PARTITION_MODEL_WITH_CFG(modelImplicit2,implicit_p,ModelConfig("implicit_p2").setBlobEnabled(true),df2Idx(),df3Idx())
...
BOOST_AUTO_TEST_SUITE(TestBlob, *boost::unit_test::fixture<HATN_TEST_NAMESPACE::DbTestFixture>())
...
BOOST_AUTO_TEST_CASE(NotPartitioned)
{
    ...
    auto handler=[&s1](std::shared_ptr<DbPlugin> plugin, std::shared_ptr<Client> client)
    {
        ...
        std::map<size_t,common::ByteArrayShared> blobs;
        for (size_t i=0;i<count+30;i++)
        {
            auto blob=common::makeShared<common::ByteArray>();
            common::Random::randContainer(*blob,0x800000,0x4000-10);
            HATN_TEST_MESSAGE_TS(fmt::format("blob {} size {}",i,blob->size()))
            blobs.emplace(i,std::move(blob));
        }

        // create objects
        auto createObjects=[&handlerCount,&oids,&client,&topics,&ok,&blobs](const auto& m, size_t count, size_t topicsCount)
        {
            for (size_t i=0;i<count;i++)
            {
                ...
                auto& blobField=o.field(base_fields::blob_field);
                blobField.set(blobs[i]);
        ...
        auto r1=client->count(modelNoP1(),topics[0]);
        if (r1)
        {
        ...
        // read first blob object
        auto r5=client->findAll(topics[0],modelNoP1());
        HATN_TEST_RESULT(r5)
        BOOST_REQUIRE(!r5);
        BOOST_REQUIRE_EQUAL(r5.value().size(),count/topics.size());
        const auto& o1=r5->at(0);
        auto bf1=o1.as<no_p::managed>();
        std::cout << "blob field size of first read object:" << bf1->field(base_fields::blob_field).value().size() << std::endl;
        const auto& bf1Buf=bf1->field(base_fields::blob_field).byteArray();
        BOOST_CHECK(bf1Buf==*blobs[0]);

        auto r2=client->count(modelNoP2(),topics[0]);
        BOOST_REQUIRE(!r2);
        ...
        auto r3=client->count(modelNoP3(),topics[0]);
        BOOST_REQUIRE(!r3);
        ...
        r2=client->count(modelNoP2());
        BOOST_REQUIRE(!r2);
        BOOST_CHECK_EQUAL(r2.value(),count+20);
        r3=client->count(modelNoP3());
        BOOST_REQUIRE(!r3);
        BOOST_CHECK_EQUAL(r3.value(),count+30);
    ...
    PrepareDbAndRun::eachPlugin(handler,"simple1.jsonc",std::vector<PartitionRange>{},true);
}
...
BOOST_AUTO_TEST_CASE(Partitions)
{
    ...
    auto handler=[&](std::shared_ptr<DbPlugin> plugin, std::shared_ptr<Client> client)
    {
        ...
        size_t maxObjectsPerPartition=80;
        size_t count=dates.size()*maxObjectsPerPartition;
        std::map<size_t,common::ByteArrayShared> blobs;
        for (size_t i=0;i<count;i++)
        {
            auto blob=common::makeShared<common::ByteArray>();
            common::Random::randContainer(*blob,0x100000,0x4000-10);
            HATN_TEST_MESSAGE_TS(fmt::format("blob {} size {}",i,blob->size()))
            blobs.emplace(i,std::move(blob));
        }
        ...
        BOOST_TEST_MESSAGE("Filling modelImplicit1...");
        size_t objectsPerPartition1=80;
        fill(modelImplicit1(),objectsPerPartition1);
        BOOST_TEST_MESSAGE("Filling modelImplicit2...");
