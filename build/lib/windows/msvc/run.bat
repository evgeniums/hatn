...
cmake -A %MSVC_BUILD_ARCH% -T %MSVC_TOOLSET% -DDEV_MODULE=%BUILD_MODULE% -DOPENSSL_ROOT_DIR=%OPENSSL_ROOT_DIR% -DBUILD_PLUGINS="%HATN_PLUGINS%" -DCMAKE_BUILD_TYPE=%BUILD_TYPE% -DINSTALL_DEV=%INSTALL_DEV% -DBUILD_STATIC=%BUILD_STATIC% -DENABLE_TRANSLATIONS=1 -DPYTHON_PATH=%PYTHON_PATH% -DGETTEXT_PATH=%GETTEXT_PATH% %USE_BOOST_COMPILER% %PROJECT_PATH%
if %errorlevel% neq 0 exit %errorlevel%
