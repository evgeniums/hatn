...
if %errorlevel% neq 0 exit %errorlevel%
rem cmake --build . --target install --config %BUILD_TYPE% -- /m:1 /p:UseMultiToolTask=true /p:MultiProcMaxCount=%BUILD_WORKERS% /fileLogger
@ECHO OFF
if %errorlevel% neq 0 exit %errorlevel%
rem xcopy install %INSTALL_DIR%\ /s /q /e /y
if %errorlevel% neq 0 exit %errorlevel%
