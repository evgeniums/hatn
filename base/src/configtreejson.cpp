...
struct ReaderHandler : public rapidjson::BaseReaderHandler<rapidjson::UTF8<>, ReaderHandler>
{
    ...
    bool setValue(T&& v)
    {
        ...
        else
        {
            ...
            {
                parser.error=fmt::format(fmt::runtime(_TR("mismatched types of array elements (note, floating numbers must contain period (.) symbol)","base")));
                return false;
            }
    ...
    ) const
{
       ...
       )
    {
        ...
        auto msg=fmt::format(fmt::runtime(_TR("{} at offset {} near {}...","base")), parser.error, offset, source.substr(offset, 32));
